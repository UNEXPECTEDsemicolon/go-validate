...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "valid struct with nested tagged fields",
			args: args{
				v: struct {
					Len      string
					InInt    int
					MinInt   int      `validate:"min:10;max:20"`
					MultiMax []string `validate:"max:10"`
					Nested   struct {
						Len        string `validate:"len:20"`
						DeepNested struct {
							MinInt int `validate:"min:10"`
					...
					InInt:    25,
					MinInt:   15,
					MultiMax: []string{"qwerty", "asdfgh", "zxcvbn"},
					Nested: struct {
						Len        string `validate:"len:20"`
						DeepNested struct {
							MinInt int `validate:"min:10"`
